...
{
    public partial class MeanGraph : Form
    {
        ...
        private void MeanGraph_Load(object sender, EventArgs e)
        {
            try
            {
                PrepareXaxisData();
        ...
        public void PlotGraph()
        {
            ...
            for (i = 0; i < 12; i++)
            {
                // ChartFlowPlot.Series["Observed"].Points.AddXY(X_axis[i], FrmThomasFieringModel.MeanY1[i]);
                ChartFlowPlot.Series["Observed"].Points.AddXY(MonthName[i], FrmThomasFieringModel.MeanY1[i]);
                
            }

            for (i = 0; i < 12; i++)
            {
                //ChartFlowPlot.Series["Predicted"].Points.AddXY(X_axis[i], FrmThomasFieringModel.MeanY2[i]);
                ChartFlowPlot.Series["Predicted"].Points.AddXY(MonthName[i], FrmThomasFieringModel.MeanY2[i]);
            }
        ...
        private void SaveGraphGridToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string ImagePath = Environment.CurrentDirectory + "\\Mean - Thomas Fiering Model" + DateTime.Now.ToString("yyyyMMddTHHmmss") + ".png";

                ChartFlowPlot.SaveImage(ImagePath, System.Windows.Forms.DataVisualization.Charting.ChartImageFormat.Png);

                MessageBox.Show("Mean - Thomas Fiering Graph Saved to: \n" + ImagePath);
            }
            catch (Exception ex)
            {
