...
namespace CSAY_RESERVOIR
{
    ...
    public partial class RegCoeffGraph : Form
    {
        int[] X_axis = new int[100];
        string[] MonthName = { "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC" };
        int i;
        public RegCoeffGraph()
        {
            InitializeComponent();
        }

        private void RegCoeffGraph_Load(object sender, EventArgs e)
        {
            try
            {
                PrepareXaxisData();
        ...
        { 
            ...
            for (i = 0; i < 12; i++)
            {
                //ChartFlowPlot.Series["Observed"].Points.AddXY(X_axis[i], FrmThomasFieringModel.RegCoef1[i]);
                ChartFlowPlot.Series["Observed"].Points.AddXY(MonthName[i], FrmThomasFieringModel.RegCoef1[i]);
            }

            for (i = 0; i < 12; i++)
            {
                // ChartFlowPlot.Series["Predicted"].Points.AddXY(X_axis[i], FrmThomasFieringModel.RegCoef2[i]);
                ChartFlowPlot.Series["Predicted"].Points.AddXY(MonthName[i], FrmThomasFieringModel.RegCoef2[i]);
            }
        ...
        private void SaveGraphGridToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string ImagePath = Environment.CurrentDirectory + "\\Reg Coef Thomas Fiering Model" + DateTime.Now.ToString("yyyyMMddTHHmmss") + ".png";

                ChartFlowPlot.SaveImage(ImagePath, System.Windows.Forms.DataVisualization.Charting.ChartImageFormat.Png);

                MessageBox.Show("Regression Coefficient - Thomas Fiering Graph Saved to: \n" + ImagePath);
            }
            catch (Exception ex)
            {
