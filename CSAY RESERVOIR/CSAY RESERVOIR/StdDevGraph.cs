...
{
    public partial class StdDevGraph : Form
    {
        ...
        private void StdDevGraph_Load(object sender, EventArgs e)
        {
            try
            {
                PrepareXaxisData();
        ...
        public void PlotGraph()
        {
            ...
            for (i = 0; i < 12; i++)
            {
                //ChartFlowPlot.Series["Observed"].Points.AddXY(X_axis[i], FrmThomasFieringModel.StdDev1[i]);
                ChartFlowPlot.Series["Observed"].Points.AddXY(MonthName[i], FrmThomasFieringModel.StdDev1[i]);
            }

            for (i = 0; i < 12; i++)
            {
                //ChartFlowPlot.Series["Predicted"].Points.AddXY(X_axis[i], FrmThomasFieringModel.StdDev2[i]);
                ChartFlowPlot.Series["Predicted"].Points.AddXY(MonthName[i], FrmThomasFieringModel.StdDev2[i]);
            }
        ...
        private void SaveGraphGridToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string ImagePath = Environment.CurrentDirectory + "\\StdDev - Thomas Fiering Model" + DateTime.Now.ToString("yyyyMMddTHHmmss") + ".png";

                ChartFlowPlot.SaveImage(ImagePath, System.Windows.Forms.DataVisualization.Charting.ChartImageFormat.Png);

                MessageBox.Show("Standard Deviation - Thomas Fiering Graph Saved to: \n" + ImagePath);
            }
            catch (Exception ex)
            {
